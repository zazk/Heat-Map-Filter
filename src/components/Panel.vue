...
export default {
  ...
  data() {
    return {
      ...
      educacion: [],
      actividades: ['Venta de repuestos.', 'Taller mecánico', 'Mantenimiento para autos mecánicos', 'Embragues', 'Motores'],
      cargos: ['Mecánico', 'Dueño', 'Ventas', 'Ayudante', 'Operador'],
      medios: ['Televisión', 'Periódico', 'Correo', 'Facebook', 'Radio', 'Celular'],
      carreras: ['Mecánica Eléctrica', 'Mecánica General', 'Mecatrónica', 'Electrónica', 'Mecánica Automotriz'],
      filtros: {
  ...
  methods: {
    ...
    addFilter(event) {
      ...
      for (var i = 0, t = this.locations.length; i < t; i++) {
        ...
        let isDistritoValid = this.filtros.distrito == 'Todos' ? true : this.filtros.distrito == loc.distrito
        let isEstadoValid = this.filtros.estado == 'Todos' ? true : this.filtros.estado == loc.estado
        let isEdadValid = loc.edad_1 >= this.filtros.edad.min && loc.edad_1 <= this.filtros.edad.max
        let isTrabajadoresValid = loc.nro_trabajadores >= parseInt(this.filtros.trabajador.min, 10) && loc.nro_trabajadores <= parseInt(this.filtros.trabajador.max, 10)
        ...
        if (this.filtros.educaciones.length > 0) {
          for (var j = 0, s = this.filtros.educaciones.length; j < s; j++) {
            ...
            isEducacion = edu == loc.educacion_1 || edu == loc.educacion_2 || edu == loc.educacion_3 || edu == loc.educacion_4
        ...
        if (this.filtros.actividades.length > 0) {
          for (var j = 0, s = this.filtros.actividades.length; j < s; j++) {
            ...
            isActividad = acti == loc.actividad_1 || acti == loc.actividad_2 || acti == loc.actividad_3 || acti == loc.actividad_4 || acti == loc.actividad_5 || acti == loc.actividad_6 || acti == loc.actividad_7 || acti == loc.actividad_8 || acti == loc.actividad_9 || acti == loc.actividad_10 || acti == loc.actividad_11 || acti == loc.actividad_12 || acti == loc.actividad_13 || acti == loc.actividad_14 || acti == loc.actividad_15 || acti == loc.actividad_16 || acti == loc.actividad_17 || acti == loc.actividad_18 || acti == loc.actividad_19 || acti == loc.actividad_20 || acti == loc.actividad_21 || acti == loc.actividad_22 || acti == loc.actividad_23 || acti == loc.actividad_24
        ...
        if (this.filtros.cargos.length > 0) {
          for (var j = 0, s = this.filtros.cargos.length; j < s; j++) {
            ...
            isCargos = acti == loc.cargo_1 || acti == loc.cargo_2 || acti == loc.cargo_3 || acti == loc.cargo_4 || acti == loc.cargo_5
        ...
        if (this.filtros.medios.length > 0) {
          for (var j = 0, s = this.filtros.medios.length; j < s; j++) {
            ...
            isMedios = acti == loc.medios_1 || acti == loc.medios_12 || acti == loc.medios_3 || acti == loc.medios_4 || acti == loc.medios_5 || acti == loc.medios_6
        ...
        if (this.filtros.carreras.length > 0) {
          for (var j = 0, s = this.filtros.carreras.length; j < s; j++) {
            ...
            isCarreras = acti == loc.carrera_1 || acti == loc.carrera_2 || acti == loc.carrera_3 || acti == loc.carrera_4 || acti == loc.carrera_5 || acti == loc.carrera_6 || acti == loc.carrera_7
        ...
        if (isDistritoValid && isEstadoValid && isEdadValid && isCargos && isMedios && isCarreras && isTrabajadoresValid && isEducacion && isActividad) {
          rows.push(loc)
          continue
        }
      }
          ...
          (function(marker, i) {
            return function() {
              ...
              $('[x-update-points]').on('submit', function(e) {
                ...
                let id = $('[x-id]')
                let url = 'https://w.areminds.com/api/parse.php?' + $('[x-update-points]').serialize()
